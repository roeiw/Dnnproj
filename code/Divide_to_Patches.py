...
with open(csv_path, "w") as csv_in:
    ...
    for f in os.listdir(path_to_data):
        ...
        for im in os.listdir(path_to_data + f):
            ...
            for i in range(int(img.size[0] / small_patch_size)):
                for j in range(int(img.size[1] / small_patch_size) ):
                    small_patch = img.crop((small_patch_size * i, small_patch_size * j, small_patch_size * (i + 1),
                                            small_patch_size * (j + 1)))
                    patch_path = im_dir_path+"/"+im_type+"_"+str(i)+"_"+str(j)
